...
namespace OCA\GoogleEncryption\Service;
...
class GooglePhotosAPIServiceCustom extends GooglePhotosAPIService {
	/**
	 * @var string
	 */
	private $appName;
			...
			do {
				...
				if (isset($result['mediaItems']) && is_array($result['mediaItems'])) {
					foreach ($result['mediaItems'] as $photo) {
						if ($this->getUserConfigValue($userId, 'importing_photos') == 0) {
							return [
